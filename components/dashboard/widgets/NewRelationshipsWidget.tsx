...
export function NewRelationshipsWidget({ profiles }: Props) {
  const items = profiles.slice(0, 5)

  return (
    <SectionCard
      title="New Relationships"
      description={items.length > 0 ? `${items.length} new contact${items.length === 1 ? "" : "s"} in the last 14 days` : undefined}
    >
      {items.length === 0 ? (
        <div className="flex items-center gap-3 rounded-[1.1rem] border border-dashed border-black/10 bg-slate-50/70 px-4 py-5">
          ...
          <p className="text-sm text-slate-500">No new relationships this period</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {items.map((person) => (
            ...
            >
              ...
              <span className="shrink-0 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-[10px] font-semibold text-blue-700">
                new
              </span>
            </article>
          ))}
